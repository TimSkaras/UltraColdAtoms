...
\label{fig:DF}
\caption{This figure plots the evolution of the MSDP for the anharmonically confined bosons and compares the initial and final MSDP to other density profiles. We can see that with the anharmonic term, the initial RSDP is larger near the center than the initial harmonic RSDP, and, correspondingly, the final MSDP is less than the initial harmonic RSDP.}
\end{figure}

\subsection*{Asymmetric Anharmonicity}
It is rather surprising to find that dynamical fermionization will still occur even when the bosons are not harmonically confined. But now we ask, what factor does the symmetry of the Hamiltonian play in this? Because the parity operator commutes with Hamiltonian \ref{anharmonic}, the resulting eigenstates are symmetric. If we add a cubic term to this Hamiltonian to obtain
\begin{equation}
